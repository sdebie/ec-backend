 ...
 * <p>
 * Resolution order: {@code CF-Connecting-IP} first, since Cloudflare always sets it
 * from the true TCP connection and clients cannot forge it through the edge; then the
 * <b>last</b> {@code X-Forwarded-For} entry, the one appended by the trusted proxy —
 * the FIRST entry is client-controlled (Cloudflare appends the real IP to any
 * client-supplied XFF), so it must never be used for rate-limit keying; then
 * {@code X-Real-IP}, then {@code "unknown"}.
 * <p>
 * Behind a self-managed (non-Cloudflare) proxy, the proxy must append or overwrite
 * X-Forwarded-For with the socket peer.
 */
