 ...
 * <p>
 * The single authorization rule for every order-scoped surface: {@link #mayAct} authorizes
...
public class OrderOwnershipGuard
{
    ...
    /**
     * The rule: a closed set of two things authorize, everything else is refused.
     * Release 1/2 share this exact body — the enforcement flag, the shadow-mode
     * fallback and its {@code WARN} log live entirely inside this method, so every
    ...
    public boolean mayAct(OrderEntity order, String presentedToken)
    {
        ...
        // its presence names a caller still not sending a token.
    ...
    /**
     * {@code mayAccess}'s original logic, renamed and made private: a permissive method
     * still called {@code mayAccess} is one a future surface could call directly, bypassing
     * the ownership check {@link #mayAct} now centralizes. It survives release 1 only, as
    ...
    /**
     * Whether the caller may have an order replayed to them. Unlike {@link #mayAct}, this
     * is keyed on the order's owner, not on the requester's authentication state: an order
     * belonging to a customer is replayable only to that customer, whether the requester is
     * anonymous or signed in as someone else. An order with no customer is replayable on
     * possession of the key alone — the same bearer model as its {@code sessionId}.
     * <p>
     * This guards idempotency replay specifically, keyed on the {@code Idempotency-Key} — a
     * different credential from the one {@link #mayAct} checks, and the caller must already
     * hold it.
     */
