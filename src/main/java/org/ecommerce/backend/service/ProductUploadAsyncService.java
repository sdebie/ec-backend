...
public class ProductUploadAsyncService {
    ...
    public void handleCsvUploadAsync(InputStream is, UUID batchId) {
        executor.submit(() -> {
            try {
                ...
                try {
                    QuarkusTransaction.requiringNew().run(() -> importService.markBatchAsFailed(batchId));
    ...
    public void processStagedRowsAsync(UUID batchId) {
        executor.submit(() -> {
            boolean retriedCachedPlanFailure = false;

            while (true) {
                try {
                    QuarkusTransaction.requiringNew().run(() -> importService.processStagedRowsForBatch(batchId));
                    QuarkusTransaction.requiringNew().run(() -> importService.markBatchAsProcessed(batchId));
                    return;
                } catch (Exception ex) {
