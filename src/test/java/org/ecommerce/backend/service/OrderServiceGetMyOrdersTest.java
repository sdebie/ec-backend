...
class OrderServiceGetMyOrdersTest {
    ...
    void getMyOrders_singleOrderWithMultipleItems_mapsCorrectly() {
        ...
        OrderEntity order = createOrder(
                ...
                List.of(2, 3, 1) // quantities — itemCount should be 6
        );
        ...
        assertEquals(order.id.toString(), dto.id);
        assertEquals(orderDate.format(DateTimeFormatter.ISO_LOCAL_DATE_TIME), dto.orderDate);
        assertEquals("PAID", dto.status);
        assertEquals(6, dto.itemCount);
        assertEquals(250.50, dto.totalAmount, 0.001);
    }
    ...
    void getMyOrders_multipleOrders_allFieldsMappedCorrectly() {
        ...
        OrderEntity order1 = createOrder(
                UUID.randomUUID(), customerId, date1,
                OrderStatusEn.DELIVERED, new BigDecimal("1500.00"), List.of(5, 2)
        );
        OrderEntity order2 = createOrder(
                UUID.randomUUID(), customerId, date2,
                OrderStatusEn.CREATED, new BigDecimal("99.99"), List.of(1)
        );
        ...
        assertEquals(order1.id.toString(), dto1.id);
        assertEquals(date1.format(DateTimeFormatter.ISO_LOCAL_DATE_TIME), dto1.orderDate);
        assertEquals("DELIVERED", dto1.status);
        assertEquals(7, dto1.itemCount); // 5 + 2
        assertEquals(1500.00, dto1.totalAmount, 0.001);

        // Second order (older)
        OrderSummaryDto dto2 = result.get(1);
        assertEquals(order2.id.toString(), dto2.id);
        assertEquals(date2.format(DateTimeFormatter.ISO_LOCAL_DATE_TIME), dto2.orderDate);
        assertEquals("CREATED", dto2.status);
        assertEquals(1, dto2.itemCount);
        assertEquals(99.99, dto2.totalAmount, 0.001);
    }
    ...
    void getMyOrders_preservesDescendingOrderFromDatabase() {
        ...
        assertEquals(newest.format(DateTimeFormatter.ISO_LOCAL_DATE_TIME), result.get(0).orderDate);
        assertEquals(middle.format(DateTimeFormatter.ISO_LOCAL_DATE_TIME), result.get(1).orderDate);
        assertEquals(oldest.format(DateTimeFormatter.ISO_LOCAL_DATE_TIME), result.get(2).orderDate);
    }
    ...
    void getMyOrders_orderWithNullCreatedAt_mapsOrderDateAsNull() {
        ...
        OrderEntity order = createOrder(
                UUID.randomUUID(), customerId, null,
                OrderStatusEn.CREATED, new BigDecimal("50.00"), List.of(1)
        );
    ...
    void getMyOrders_orderWithNullStatus_mapsStatusAsNull() {
        ...
        OrderEntity order = createOrder(
                ...
                new BigDecimal("75.00"), List.of(2)
        );
    ...
    void getMyOrders_orderWithNullTotalAmount_mapsTotalAmountAsZero() {
        ...
        OrderEntity order = createOrder(
                UUID.randomUUID(), customerId,
                LocalDateTime.now(), OrderStatusEn.PENDING,
                null, List.of(3)
        );
    ...
    void getMyOrders_orderItemWithNullQuantity_treatsAsZero() {
        ...
        item2.quantity = null; // null quantity
        item2.orderEntity = order;

        order.items = new ArrayList<>(List.of(item1, item2));
        ...
        assertEquals(1, result.size());
        assertEquals(4, result.get(0).itemCount); // 4 + 0 (null treated as 0)
    }

    // --- Helper methods ---

    private OrderEntity createOrder(UUID orderId, UUID customerId, LocalDateTime createdAt,
                                    OrderStatusEn status, BigDecimal totalAmount,
