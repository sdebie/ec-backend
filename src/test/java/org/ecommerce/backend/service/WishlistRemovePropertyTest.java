...
class WishlistRemovePropertyTest {
    ...
    private static class WishlistSimulator {
        ...
        void seedWishlistEntry(UUID customerId, UUID variantId) {
            ...
            if (!wishlistEntries.containsKey(key)) {
                ...
                item.customer = customers.get(customerId);
                item.variant = variants.get(variantId);
        ...
        List<UUID> getVariantIdsForCustomer(UUID customerId) {
            ...
            for (Map.Entry<String, WishlistItemEntity> entry : wishlistEntries.entrySet()) {
                if (entry.getKey().startsWith(prefix)) {
                    result.add(entry.getValue().variant.id);
                }
            }
            return result;
        }
     ...
     */
    @Property(tries = 100)
    void removingExistingVariantRemovesItFromWishlist(
            @ForAll("randomUUID") UUID customerId,
            @ForAll("initialWishlistVariants") List<UUID> initialVariants,
            @ForAll("removeCount") int removeCount
    ) {
        ...
        assertTrue(simulator.containsEntry(customerId, variantToRemove),
                "Variant should exist in wishlist before removal");
        ...
        assertFalse(simulator.containsEntry(customerId, variantToRemove),
                "Variant should NOT appear in wishlist after removal");
        ...
        assertFalse(remainingVariants.contains(variantToRemove),
                "Removed variant should not be in the customer's wishlist variant IDs");
    }
     ...
     */
    @Property(tries = 100)
    void removingNonExistentVariantSucceedsAndLeavesWishlistUnchanged(
            @ForAll("randomUUID") UUID customerId,
            @ForAll("initialWishlistVariants") List<UUID> initialVariants,
            @ForAll("randomUUID") UUID variantToRemove,
            @ForAll("removeCount") int removeCount
    ) {
        ...
        assertFalse(simulator.containsEntry(customerId, variantToRemove),
                "Variant should NOT appear in wishlist after removal (was never present)");

        // Assert: wishlist is unchanged — all original entries remain
        assertEquals(entriesBefore, simulator.totalEntries(),
                "Total entries should not change when removing non-existent variant");

        List<UUID> variantsAfter = simulator.getVariantIdsForCustomer(customerId);
        assertEquals(new HashSet<>(variantsBefore), new HashSet<>(variantsAfter),
                "Wishlist variant IDs should remain unchanged when removing non-existent variant");
    }
     ...
     */
    @Property(tries = 100)
    void variantAbsentAfterRemovalRegardlessOfInitialState(
            @ForAll("randomUUID") UUID customerId,
            @ForAll("initialWishlistVariants") List<UUID> initialVariants,
            @ForAll("randomUUID") UUID variantToRemove
    ) {
        ...
        assertFalse(simulator.containsEntry(customerId, variantToRemove),
                "Variant must not appear in wishlist after removal, regardless of initial state");

        List<UUID> remainingVariants = simulator.getVariantIdsForCustomer(customerId);
        assertFalse(remainingVariants.contains(variantToRemove),
                "Variant must not be in variant ID list after removal");
    }
    ...
    Arbitrary<List<UUID>> initialWishlistVariants() {
        ...
        Arbitrary<UUID> uuidArb = Combinators.combine(
                Arbitraries.longs(),
                Arbitraries.longs()
        ).as(UUID::new);
