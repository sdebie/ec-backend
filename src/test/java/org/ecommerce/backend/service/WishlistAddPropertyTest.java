...
class WishlistAddPropertyTest
{
     ...
     */
    @Property(tries = 100)
    void addingExistingVariantNTimesResultsInExactlyOneEntry(@ForAll("randomUUID") UUID customerId, @ForAll("randomUUID") UUID variantId, @ForAll("addCount") int addCount)
    {
        ...
        assertEquals(WishlistService.AddResult.CREATED, firstResult, "First add should return CREATED");

        // Assert: exactly one CREATED result across all adds
        assertEquals(1, createdCount, "Exactly one add should return CREATED");

        // Assert: remaining adds return ALREADY_EXISTS
        assertEquals(addCount - 1, alreadyExistsCount, "All subsequent adds should return ALREADY_EXISTS");

        // Assert: exactly one entry exists for this (customer, variant) pair
        assertEquals(1, simulator.entryCount(customerId, variantId), "Exactly one entry should exist for (customer, variant) pair after N adds");

        // Assert: total entries is exactly 1
        assertEquals(1, simulator.totalEntries(), "Total wishlist entries should be exactly 1");
    }
     ...
     */
    @Property(tries = 100)
    void addingNonExistentVariantReturnsNotFoundAndNoChange(@ForAll("randomUUID") UUID customerId, @ForAll("randomUUID") UUID variantId, @ForAll("addCount") int addCount)
    {
        WishlistSimulator simulator = new WishlistSimulator();
        simulator.addCustomer(customerId);
        ...
        for (int i = 0; i < addCount; i++) {
            ...
            assertEquals(WishlistService.AddResult.VARIANT_NOT_FOUND, result, "Add with non-existent variant should always return VARIANT_NOT_FOUND");
        }

        // Assert: no entries were created (wishlist unchanged)
        assertEquals(entriesBefore, simulator.totalEntries(), "Wishlist should remain unchanged when variant does not exist");

        // Assert: specifically no entry for this (customer, variant) pair
        assertEquals(0, simulator.entryCount(customerId, variantId), "No entry should exist for non-existent variant");
    }
